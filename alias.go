...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    echo ""
    echo "Arguments:"
    echo "  target    Target machine name (e.g., prod, staging, web1)"
    ...
    echo ""
    echo "Examples:"
    echo "  rcx prod 'hostname'"
    echo "  rcx staging 'uptime' --stream"
    echo "  rcx web1 'ls -la ~' 20"
    echo "  rcx --targets prod,staging,web1 'hostname'"
...
if [ $# -lt 2 ]; then
    ...
    echo "Example: rcx prod 'hostname'"
    echo ""
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    echo "  prod                  up      2min ago          prod-server-01"
    echo "  staging               down    checked 1h ago    target not connected"
    exit 0
fi
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    echo ""
    echo "Examples:"
    echo "  rcc prod /path/to/file.txt /remote/path/file.txt"
    echo "  rcc staging /path/to/dir /remote/path/dir"
    echo "  rcc web1 ~/config /home/user/config --stream"
    echo ""
    echo "Available targets: use 'rcl' to list configured targets"
    exit 0
fi
...
if [ $# -lt 3 ]; then
    ...
    echo ""
    echo "Usage: rcc <target> <src> <dst> [--stream]"
    echo "Example: rcc prod /path/to/file.txt /remote/path/file.txt"
    echo ""
