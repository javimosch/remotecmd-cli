...
func helpCatalog() map[string]any {
	c := map[string]catalogCommand{
		...
		"daemon start": {Auth: false,
			Flags:   []string{"--token <t>", "-daemon"},
			Summary: "run the target daemon on this machine (dials out to the relay)"},
		"daemon stop":   {Summary: "stop the background target daemon"},
		"daemon status": {Summary: "show the background target daemon's state"},
		"daemon " + persistenceSubcommandName(): {Args: []string{"install|remove"},
			Summary: "install or remove the daemon as a boot service"},
		"relay daemon start": {Flags: []string{"--port <n>", "-daemon", "--tls-cert <file>", "--tls-key <file>"},
			Summary: "run the relay hub (listens on all interfaces)"},
		"relay daemon stop":   {Summary: "stop the background relay"},
		"relay daemon status": {Summary: "show the background relay's state"},
